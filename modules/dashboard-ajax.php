...
function sola_st_db_ajax_callback(){
	...
	if( isset( $_POST['action'] ) ){
		...
		if( $_POST['action'] == 'sola_st_db_request_tickets_from_control_priority' ){
			...
			$required_action = sanitize_textarea_field( $_POST['required_action'] );
		...
		if( $_POST['action'] == 'sola_st_db_request_ticket_from_content_list' ){
			...
			$ret .= "			<div class='ticket_author'>$ticket_author_name | <span>$ticket_request_date</span> | <div class='ticket_id_label'>".__('Ticket #', 'sola_st')."$ticket_id</div></div>";
