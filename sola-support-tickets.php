...
/* 
 * 3.18 - 2017-02-22 - High Priority
 * All Ajax and settings variables are sanitized and escaped properly
 * Paths to files are no longer hard coded in
 * 
 * 3.17 - 2017-02-17 - High priority
 * PHPMailer vulnerability fix
 * 
...
function sola_st_show_pending_number( $menu ) {
    ...
    $settings = get_option('sola_st_settings');

    if( isset( $settings['sola_st_display_legacy_tickets'] ) && $settings['sola_st_display_legacy_tickets'] == 1 ){
...
function sola_st_admin_menu() {
    ...
    if( isset( $settings['sola_st_display_legacy_tickets'] ) && $settings['sola_st_display_legacy_tickets'] == 1 ){
        ...
        add_submenu_page('support-tickets', __('Log', 'sola'), __('System Log', 'sola_st'), 'manage_options', 'sola-st-menu-error-log', 'sola_st_admin_error_log_layout');
    }    
}
...
function sola_st_admin_scripts_basic() {
     ...
     * @author  Sola Plugins, Jarryd Long
     * @since  4.0.0
     */
    ...
    if( isset( $_GET['page'] ) && $_GET['page'] == 'support-tickets' ){
        ...
        wp_register_script('sola-st-dashboard', plugins_url('js/dashboard.js', __FILE__), array('jquery'), '', true);

        $sola_st_dashboard_nonce = wp_create_nonce("sola_st_dashboard");

        wp_localize_script( 'sola-st-dashboard', 'sola_st_dashboard_security', $sola_st_dashboard_nonce );
        wp_localize_script( 'sola-st-dashboard', 'sola_st_db_plugins_url', plugins_url() );

        wp_enqueue_script('sola-st-dashboard');
        
        wp_register_style('sola-st-dashboard-styles', plugins_url('/css/dashboard.css', __FILE__));
        wp_enqueue_style('sola-st-dashboard-styles');
...
function sola_st_wp_head() {
        ...
        if (isset($_POST['sola_st_save_settings'])) {
            ...
            if ( ! empty( $_POST ) && check_admin_referer( 'sola_st_save_admin_settings_basic', 'sola_st_security' ) && current_user_can( 'manage_options' ) ) {
                ...
                if (isset($_POST['sola_st_settings_notify_new_tickets'])) {
                    $sola_st_settings['sola_st_settings_notify_new_tickets'] = sanitize_text_field( $_POST['sola_st_settings_notify_new_tickets'] );
                } else {
                    $sola_st_settings['sola_st_settings_notify_new_tickets'] = 0;
                }
                if (isset($_POST['sola_st_settings_notify_new_responses'])) {
                    $sola_st_settings['sola_st_settings_notify_new_responses'] = sanitize_text_field( $_POST['sola_st_settings_notify_new_responses'] );
                } else {
                    $sola_st_settings['sola_st_settings_notify_new_responses'] = 0;
                }
                if (isset($_POST['sola_st_settings_allow_html'])) {
                    $sola_st_settings['sola_st_settings_allow_html'] = sanitize_text_field( $_POST['sola_st_settings_allow_html'] );
                } else {
                    $sola_st_settings['sola_st_settings_allow_html'] = 0;
                }
                if (isset($_POST['sola_st_settings_thank_you_text'])) {
                    $sola_st_settings['sola_st_settings_thank_you_text'] = sanitize_text_field( $_POST['sola_st_settings_thank_you_text'] );
                } else {
                    $sola_st_settings['sola_st_settings_thank_you_text'] = __('Thank you for submitting your support ticket. One of our agents will respond as soon as possible.', 'sola_st');
                }
                if (isset($_POST['sola_st_settings_allow_priority'])) {
                    $sola_st_settings['sola_st_settings_allow_priority'] = sanitize_text_field( $_POST['sola_st_settings_allow_priority'] );
                } else {
                    $sola_st_settings['sola_st_settings_allow_priority'] = 0;
                }
                ...
                {
                    $sola_st_settings['sola_st_settings_default_priority'] = sanitize_text_field( $_POST['sola_st_settings_default_priority'] );
                } 
                else 
                {
                ...
                if (isset($_POST['sola_st_settings_notify_status_change'])) {
                    $sola_st_settings['sola_st_settings_notify_status_change'] = sanitize_text_field( $_POST['sola_st_settings_notify_status_change'] );
                } else {
                    $sola_st_settings['sola_st_settings_notify_status_change'] = 0;
                }
        		...
        		{
        			$rb_sola_mailing_system_selection = sanitize_text_field( $_REQUEST['rb_sola_mailing_system_selection'] );
        			if($rb_sola_mailing_system_selection==='smtp')
        			{
        				if( isset( $_REQUEST['sola_st_smtp_host_setting_php_mailer'] ) && isset( $_REQUEST['sola_st_smtp_username_setting_php_mailer'] ) && isset( $_REQUEST['sola_st_smtp_password_setting_php_mailer'] ) && isset( $_REQUEST['sola_st_smtp_port_setting_php_mailer'] ) && isset( $_REQUEST['sola_st_smtp_encryption_setting_php_mailer'] ) ){
        					...
        					$sola_st_settings['sola_st_smtp_host_setting_php_mailer'] = sanitize_text_field( $_REQUEST['sola_st_smtp_host_setting_php_mailer'] );
        					$sola_st_settings['sola_st_smtp_username_setting_php_mailer'] = sanitize_text_field( $_REQUEST['sola_st_smtp_username_setting_php_mailer'] );
        					$sola_st_settings['sola_st_smtp_password_setting_php_mailer'] = sanitize_text_field( $_REQUEST['sola_st_smtp_password_setting_php_mailer'] );
        					$sola_st_settings['sola_st_smtp_port_setting_php_mailer'] = sanitize_text_field( $_REQUEST['sola_st_smtp_port_setting_php_mailer'] );
        					$sola_st_settings['sola_st_smtp_encryption_setting_php_mailer'] = sanitize_text_field( $_REQUEST['sola_st_smtp_encryption_setting_php_mailer'] );
        		...
        		{
        			$sola_st_settings['sola_st_automated_emails_from_name'] = sanitize_text_field( $data['sola_st_automated_emails_from_name'] );
        		}
        		
        		if(isset($data['sola_st_mailbox_cron_frequency']))
        		{
        			$sola_st_settings['sola_st_mailbox_cron_frequency'] = sanitize_text_field( $data['sola_st_mailbox_cron_frequency'] );
        		}
        		
        		if(isset($data['sola_st_automated_emails_from']))
        		{
        			$sola_st_settings['sola_st_automated_emails_from'] = sanitize_text_field( $data['sola_st_automated_emails_from'] );
        		}
    ...
    if (isset($_POST['sola_st_send_feedback'])) {
         ...
         if ( ! empty( $_POST ) && check_admin_referer( 'sola_st_save_admin_settings_basic', 'sola_st_security' ) && current_user_can( 'manage_options' ) ) {
    		...
    		{
    			$mail_result = send_automated_emails( "support@solaplugins.com", "Support Tickets Plugin feedback", "Name: " . sanitize_text_field( $_POST['sola_st_feedback_name'] ) . " <br/><br/> " . "Email: " . sanitize_text_field( $_POST['sola_st_feedback_email'] ) . " <br/><br/> " . "Website: " . sanitize_text_field( $_POST['sola_st_feedback_website'] ) . " <br/><br/> " . "Feedback:" . sanitize_text_field( $_POST['sola_st_feedback_feedback'] ) );
    		}
    		else
    		{
    			$mail_result=false;		
...
function sola_st_return_error_log() {    
    ...
    $fh = @fopen( $upload_dir['basedir'] . '/sola' . "/sola_st_log.txt", "r");
    $ret = "";
    if ($fh) {
        for ($i = 0; $i < 10; $i++) {
    ...
    if ($check == 1) {
        ...
        if ($_POST['action'] == "sola_st_save_response") {
            ...
            if( isset( $_POST['base_64_data'] ) && isset( $_POST['file_name'] ) && isset( $_POST['file_mime_type'] ) ){
                ...
                $posted_full_base_64 = sanitize_text_field( $_POST['base_64_data'] );
                ...
                if( !function_exists( 'wp_get_current_user' ) ){
                     ...
                     */
                    require_once( admin_url( 'wp-includes/pluggable.php' ) );
                }
    ...
    if ($type == 'response') {
        ...
        if (isset($user_email)) {
            ...
            if ($sola_st_settings['sola_st_username'] && function_exists('sola_st_pro_init')) {
                $admin_address = esc_attr( $sola_st_settings['sola_st_username'] );
            } else {
            ...
            $additional_response = esc_attr( $sola_st_settings['sola_st_settings_thank_you_text'] );
...
function sola_st_append_responses_to_ticket($post_id) {
    ...
    if ($custom['ticket_status'][0] == "1") {
        ...
        if (is_user_logged_in()) {
            ...
            if( function_exists( 'sola_st_pro_display_uploaded_files' ) ){
                $add_a_response .= '
                    <tr>
                       <td>
                            '.__('Uploaded Files: ', 'sola_st').sola_st_pro_display_uploaded_files($ticket_id).' 
                       </td>
                    </tr>';
            }
            $add_a_response .= '<div class="sola_st_response_div">
...
function sola_st_draw_response_box($post_id) {
    ...
    $role = implode(', ', $author_data->roles);
...
function sola_st_user_head() {
    ...
    if (isset($_POST['sola_st_submit_ticket']) && $_POST['sola_st_ticket_title'] != "") {
        ...
        $content = sola_st_check_for_html( sanitize_text_field( $_POST['sola_st_ticket_text'] ) );

        if(isset($_POST['sola_st_submit_department']))
        {
            $tax_input = array(
                'sola_st_deparments' => wp_strip_all_tags( sanitize_text_field( $_POST['sola_st_submit_department'] ) )
            );
        }
        ...
        $data = array(
            'post_content' => $content,
            'post_status' => 'publish',
            'post_title' => sanitize_text_field( $_POST['sola_st_ticket_title'] ),
        ...
        if (isset($_POST['sola_st_submit_priority'])) {
            add_post_meta($post_id, 'ticket_priority', sanitize_text_field( $_POST['sola_st_submit_priority'] ), true);
        } else {
            /* get default */
            $sola_default_priority = esc_attr( $sola_st_settings['sola_st_settings_default_priority'] );
    ...
    if (isset($_POST['sola_st_response_id']) && $_POST['sola_st_response_id'] != "") {
        ...
        $parent_id = sanitize_text_field( $_POST['sola_st_response_id'] );
        ...
        $data = array(
            'post_content' => $content,
            'post_status' => 'publish',
            'post_title' => sanitize_text_field( wp_strip_all_tags( $_POST['sola_st_response_title'] ) ),
            'post_type' => 'sola_st_responses',
            'post_author' => get_current_user_id(),
            'comment_status' => 'closed',
            'ping_status' => 'closed'
        ...
        if (isset($query->query['post_type']) && $query->query['post_type'] == "sola_st_tickets") {
            ...
            if (isset($_GET['sola_st_agent_mk']) and isset($_GET['sola_st_agent_mv']) and ( $_GET['sola_st_agent_mv'] != '')) {
                ...
                $agent_array = array(
                    ...
                    'value' => sanitize_text_field( $_GET['sola_st_agent_mv'] )
                );
            } else {
                $agent_array = array('' => '');
            }
            if (isset($_GET['sola_st_priority_mk']) and isset($_GET['sola_st_priority_mv']) and ( $_GET['sola_st_priority_mv']) != '') {
                ...
                $priority_array = array(
                    'key' => 'ticket_priority',
                    'value' => sanitize_text_field( $_GET['sola_st_priority_mv'] )
                );
            } else {
                $priority_array = array('' => '');
            }
            if (isset($_GET['sola_st_status_mk']) and isset($_GET['sola_st_status_mv']) and ( $_GET['sola_st_status_mv']) != '') {
                ...
                if ($_GET['sola_st_status_mv'] == "99") {
                    ...
                    $status_code = sanitize_text_field( $_GET['sola_st_status_mv'] );
                }
                $status_array = array(
                    'key' => 'ticket_status',
                    'value' => "$status_code"
...
function sola_st_select_mailing_system_to_use()
{
		...
		{
			$rb_sola_mailing_system_selection = esc_attr( $data['rb_sola_mailing_system_selection'] );
		}
		
		if(isset($data['sola_st_smtp_host_setting_php_mailer']))
		{
			$sola_st_smtp_host_setting_php_mailer = esc_attr( $data['sola_st_smtp_host_setting_php_mailer'] );
		...
		if(isset($data['sola_st_smtp_username_setting_php_mailer']))
		{
			...
			$sola_st_smtp_username_setting_php_mailer = esc_attr( $data['sola_st_smtp_username_setting_php_mailer'] );
		}
		
		if(isset($data['sola_st_smtp_password_setting_php_mailer']))
		{
			$sola_st_smtp_password_setting_php_mailer = esc_attr( $data['sola_st_smtp_password_setting_php_mailer'] );
		}
		
		if(isset($data['sola_st_smtp_port_setting_php_mailer']))
		{
			$sola_st_smtp_port_setting_php_mailer = esc_attr( $data['sola_st_smtp_port_setting_php_mailer'] );
		}
		
		if(isset($data['sola_st_smtp_encryption_setting_php_mailer']))
		{
			$sola_st_smtp_encryption_setting_php_mailer = esc_attr( $data['sola_st_smtp_encryption_setting_php_mailer'] );
		}
	...
	if($headers===null)
	{
		...
		if(function_exists("sola_st_pro_activate"))
		{
			if((isset($data['sola_st_automated_emails_from'])&& filter_var($data['sola_st_automated_emails_from'], FILTER_VALIDATE_EMAIL) !== false)&&(isset($data['sola_st_automated_emails_from_name'])&&ctype_alnum($data['sola_st_automated_emails_from_name'])===true))
			{
				...
				$replyto_address = esc_attr( $data['sola_st_automated_emails_from'] );
            	$from_address = esc_attr( $data['sola_st_automated_emails_from'] );
            	$from_name = esc_attr( $data['sola_st_automated_emails_from_name'] );
            	$replyto_name = esc_attr( $data['sola_st_automated_emails_from_name'] );
...
/** New code added in 2016 by Jarryd Long */

function sola_st_modern_dashboard(){
...
function sola_st_basic_pro_filtering_functionality(){
    ...
    $ret .= "<div class='sola_st_db_column'>";

    $ret .= "<h4>". __('Filter By Ticket Privacy', 'sola_st')."</h4>";

    $ret .= "<p>".sprintf(__('Filtering tickets according to their privacy is only available in the %s', 'sola_st'), '<a href="http://solaplugins.com/plugins/sola-support-tickets-helpdesk-plugin/?utm_source=plugin&utm_medium=link&utm_campaign=st_ticket_filter_privacy" target="_BLANK">'.__('Pro Version', 'sola_st').'</a>')."</p>";

    $ret .= "</div>";

    $ret .= "<div class='sola_st_db_column'>";

    $ret .= "<h4>". __('Filter By Assigned Agent', 'sola_st')."</h4>";

    $ret .= "<p>".sprintf(__('Filtering tickets according to the agent that they are assigned to is only available in the %s', 'sola_st'), '<a href="http://solaplugins.com/plugins/sola-support-tickets-helpdesk-plugin/?utm_source=plugin&utm_medium=link&utm_campaign=st_ticket_filter_agents" target="_BLANK">'.__('Pro Version', 'sola_st').'</a>')."</p>";

    $ret .= "</div>";

    $ret .= "<div class='sola_st_db_column'>";

    $ret .= "<h4>". __('Filter By Department', 'sola_st')."</h4>";

    $ret .= "<p>".sprintf(__('Filtering tickets according to the department they are assigned to is only available in the %s', 'sola_st'), '<a href="http://solaplugins.com/plugins/sola-support-tickets-helpdesk-plugin/?utm_source=plugin&utm_medium=link&utm_campaign=st_ticket_filter_departments" target="_BLANK">'.__('Pro Version', 'sola_st').'</a>')."</p>";
...
function sola_st_internal_notes_submit( $string, $data ){
...
add_filter('sola_st_current_agent_meta', 'sola_st_response_type_switch', 10, 3);

function sola_st_response_type_switch( $string, $data ){
